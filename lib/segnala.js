...
// SEGNALAZIONE DAL CODICE (23/09/2026). Serve ai punti in cui un'operazione
// automatica fallisce senza che nessuno la veda (stato non riportato all'app
// agenti, archiviazione automatica, imponibile non scritto). Una riga in
// `segnalazioni` avvisa Luca su Telegram: meglio un avviso in piu' di un ponte
// rotto per giorni. Deduplica per chiave nella sessione: lo stesso guasto non
// manda dieci avvisi in dieci minuti.
const gia = new Set()
export async function segnalaDaCodice(app, chiave, testo) {
  if (gia.has(chiave)) return false
  gia.add(chiave)
  try { await invia(app, 'sistema', `[${chiave}] ${testo}`); return true } catch { return false }
}
